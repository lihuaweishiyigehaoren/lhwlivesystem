...
private slots:
    void onsigChatName(User user);//添加用户名到列表
    void onsigChatList(User user);//刷新列表
    void on_chatlist_btn_clicked();//刷新按钮
    void onsigMasterExit(User user);//房主退出
    void on_send_clicked();//发送信息
    void onsigChatText(User user);//文字聊天信息处理
    void on_video_clicked();//视频
    void onsigVideo(User user);//视频处理
    void onsigunVideo(User user);//关闭视频
    void UdpReadRead();//udp处理槽函数
    void onvideoChanged(QVideoFrame);//视频采集成图片
    void onTimeOut();//定时器槽函数
    void onsigBs(User user);//弹幕处理
