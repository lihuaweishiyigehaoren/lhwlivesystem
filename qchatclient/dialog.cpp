...
/*
函数功能：构造函数
参数：ClientSocket *cs, QTcpSocket *socket, User user, QWidget *parent
返回值：无
*/
Dialog::Dialog(ClientSocket *cs, /*QTcpSocket *socket,*/ User user, QWidget *parent) :
    cs(cs), /*socket(socket),*/
    ...
    ui(new Ui::Dialog)
{
    ...
    //显示聊天室列表
    showRoomName();
    QObject::connect (cs, SIGNAL(sigRoomName(User)),
                      this, SLOT(onsigRoomName(User)));
    //刷新聊天室
    QObject::connect (cs, SIGNAL(sigRoomList(User)),
                      this, SLOT(onsigRoomList(User)));
...
返回值：无
*/
void Dialog::onsigRoomName(User user)
{
...
返回值：无
*/
void Dialog::onsigRoomList(User user)
{
